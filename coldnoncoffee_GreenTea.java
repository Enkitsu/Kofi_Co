...
public class coldnoncoffee_GreenTea extends JFrame {
	...
	JPanel enhancersPanel = new JPanel();
	...
	public String selectedSweetness = "";
	public String selectedEnhancer = "";
	...
	public void BottomHeader() {
		...
		ActionListener proceedAction = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				StringBuilder orderName = new StringBuilder("Green Tea");
				if (!selectedSize.isEmpty())
					orderName.append(" - ").append(selectedSize);
				if (!selectedSweetness.isEmpty())
					orderName.append(", ").append(selectedSweetness);
				if (!selectedEnhancer.isEmpty() && !selectedEnhancer.equals("None"))
					orderName.append(", ").append(selectedEnhancer);
	...
	public void setupAddOnPanel() {
		...
		String[] sizePrice = { "120.00", "130.00", "140.00" };

		sizeOptPanel.add(setupOptionsPanel(1, 3, 120, 40, 400, 3, sizeIMG, sizeOption, sizePrice));

		sweetnessPanel.setLayout(new BoxLayout(sweetnessPanel, BoxLayout.Y_AXIS));
		sweetnessPanel.add(optionHeaderPanel("Sweetness Level: ", "Pick 1"));

		JPanel sweetnessOptPanel = new JPanel();
		sweetnessOptPanel.setLayout(new BoxLayout(sweetnessOptPanel, BoxLayout.Y_AXIS));

		String[] sweetnessIMG = { "", "", "", "" };
		String[] sweetnessOption = { "No Sugar", "Less Sweet", "Regular", "Extra Sweet" };
		String[] sweetnessPrice = { "", "", "", "" };

		sweetnessOptPanel.add(setupOptionsPanel(2, 2, 150, 40, 250, 4, sweetnessIMG, sweetnessOption, sweetnessPrice));

		enhancersPanel.setLayout(new BoxLayout(enhancersPanel, BoxLayout.Y_AXIS));
		enhancersPanel.add(optionHeaderPanel("Enhancers: ", "Pick 1"));

		JPanel enhancersOptPanel = new JPanel();
		enhancersOptPanel.setLayout(new BoxLayout(enhancersOptPanel, BoxLayout.Y_AXIS));

		String[] enhancerIMG = { "", "", "" };
		String[] enhancerOption = { "Lemon Slice", "Honey", "Mint" };
		String[] enhancerPrice = { "15.00", "15.00", "15.00" };

		enhancersOptPanel.add(setupOptionsPanel(1, 3, 150, 40, 500, 3, enhancerIMG, enhancerOption, enhancerPrice));
		...
		addOnPanel.add(sweetnessPanel);
		addOnPanel.add(sweetnessOptPanel);

		addOnPanel.add(enhancersPanel);
		addOnPanel.add(enhancersOptPanel);

		sizePanel.setOpaque(false);
		sizeOptPanel.setOpaque(false);
		sweetnessPanel.setOpaque(false);
		sweetnessOptPanel.setOpaque(false);
		enhancersPanel.setOpaque(false);
		enhancersOptPanel.setOpaque(false);
				...
				public void actionPerformed(ActionEvent e) {
					if (hasLabelP) {
						...
						if (Arrays.asList("Small", "Medium", "Large").contains(labelB)) {
							selectedSize = labelB;
						} else if (Arrays.asList("No Sugar", "Less Sweet", "Regular", "Extra Sweet").contains(labelB)) {
							selectedSweetness = labelB;
						} else if (Arrays.asList("Lemon Slice", "Honey", "Mint").contains(labelB)) {
							selectedEnhancer = labelB;
						}
					}
					updatePrice();
				}
	...
	public void setupDescription() {
		name = "<html><p style='text-align: center;'>Green Tea</p></html>";
		desc = "<html><p style='width: 450px; text-align: center;'>A refreshing and healthy beverage "
				+ "featuring premium Japanese green tea leaves, served over ice with your choice of "
				+ "sweetness and natural enhancers.</p></html>";
	}
