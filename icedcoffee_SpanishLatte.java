...
public class icedcoffee_SpanishLatte extends JFrame {
	...
	JPanel espressoPanel = new JPanel();
	...
	public String selectedEspresso = "";
	...
	public icedcoffee_SpanishLatte() {
		mainFrame();
		mainPanel();
		frame.add(mainPanel);
		frame.pack();
	...
	public void BottomHeader() {
		...
		ActionListener proceedAction = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				StringBuilder orderName = new StringBuilder("Iced Spanish Latte");
				if (!selectedSize.isEmpty())
					orderName.append(" - ").append(selectedSize);
				if (!selectedEspresso.isEmpty())
					orderName.append(", ").append(selectedEspresso);
	...
	public double calculateSingleItemPrice() {
		...
		if (!selectedSyrup.isEmpty() && !selectedSyrup.equals("None"))
			price += 10.0;
		if (!selectedMilk.isEmpty() && !selectedMilk.equals("None"))
			price += 15.0;
	...
	public void setupAddOnPanel() {
		...
		String[] sizePrice = { "145.00", "155.00", "165.00" };

		sizeOptPanel.add(setupOptionsPanel(1, 3, 120, 40, 400, 3, sizeIMG, sizeOption, sizePrice));

		espressoPanel.setLayout(new BoxLayout(espressoPanel, BoxLayout.Y_AXIS));
		espressoPanel.add(optionHeaderPanel("Espresso Shot: ", "Pick 1"));

		JPanel espressoOptPanel = new JPanel();
		espressoOptPanel.setLayout(new BoxLayout(espressoOptPanel, BoxLayout.Y_AXIS));

		String[] espressoIMG = { "", "", "" };
		String[] espressoOption = { "Regular", "Decaf", "None" };
		String[] espressoPrice = { "", "", "" };

		espressoOptPanel.add(setupOptionsPanel(1, 3, 150, 40, 500, 3, espressoIMG, espressoOption, espressoPrice));
		...
		String[] syrupIMG = { "", "", "", "" };
		String[] syrupsOption = { "Vanilla", "Caramel", "Hazelnut", "Salted Caramel" };
		String[] syrupsPrice = { "10.00", "10.00", "10.00", "10.00" };

		syrupsOptPanel.add(setupOptionsPanel(2, 3, 200, 40, 400, 4, syrupIMG, syrupsOption, syrupsPrice));
		...
		String[] milkPrice = { "15.00", "15.00", "15.00", "15.00" };

		milkOptPanel.add(setupOptionsPanel(2, 3, 200, 40, 400, 4, milkIMG, milkOption, milkPrice));
		...
		addOnPanel.add(espressoPanel);
		addOnPanel.add(espressoOptPanel);
		...
		espressoPanel.setOpaque(false);
		espressoOptPanel.setOpaque(false);
				...
				public void actionPerformed(ActionEvent e) {
					if (hasLabelP) {
						...
						if (Arrays.asList("Small", "Medium", "Large").contains(labelB)) {
							selectedSize = labelB;
						} else if (Arrays.asList("Regular", "Decaf", "None").contains(labelB)) {
							selectedEspresso = labelB;
						} else if (Arrays.asList("Vanilla", "Caramel", "Hazelnut", "Salted Caramel").contains(labelB)) {
							selectedSyrup = labelB;
						} else if (Arrays
								.asList("Whole Milk", "Chocolate Drizzle", "Caramel Drizzle", "Cinnamon Powder")
								.contains(labelB)) {
							selectedMilk = labelB;
						}
					}
	...
	public void setupDescription() {
		name = "<html><p style='text-align: center;'>Iced Spanish Latte</p></html>";
		desc = "<html><p style='width: 450px; text-align: center;'>A luxurious Spanish-inspired coffee "
				+ "featuring rich espresso and condensed milk over ice, creating a perfectly "
				+ "balanced sweet and creamy coffee experience.</p></html>";
	}
